  ...
  try {
    ...
    if (!result) {
      return res.status(500).json({ 
        message: 'Scraping not available in production environment',
        fallback: true 
      });
    }

    res.json({ 
      message: `Successfully scraped properties for ${city}`,
      count: result.length || 0 
    });
  } catch (error) {
    console.error('Error scraping properties:', error);
    res.status(500).json({ message: 'Failed to scrape properties' });
  }
}